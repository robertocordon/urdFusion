...
- `urdFusion.py` — Fusion add-in entry point. Owns the toolbar button, command/event handler scaffolding, and module reloads. The Fusion-required `run(context)` and `stop(context)` live here. Does **not** contain export logic. The toolbar button is registered in `SolidScriptsAddinsPanel` with `isPromoted = True` (appears directly in the toolbar bar, not buried in the dropdown) and a `resourceFolder` pointing to `resources/` (contains `16x16.png`, `32x32.png`, `64x64.png` for the toolbar icon — replace the placeholder solid-color PNGs with real artwork).
...
- `modules/urdfJoint.py` — Joint data model and collection logic. Defines `JointParams(damping, friction, effort, velocity)`, `VisualOrigin(xyz: tuple, rpy: tuple)`, and `JointData`. `JointParams` holds per-joint simulation parameters: `damping` and `friction` default to `None` (omitted from URDF when absent); `effort` and `velocity` default to `_DEFAULT_EFFORT`/`_DEFAULT_VELOCITY` (100.0) and are always present. Users set these by appending `||key=value` pairs to the joint name in Fusion's browser (e.g. `shoulder_revolute||d=0.5 e=50 v=3.14`); valid keys are `d`, `f`, `e`, `v`. Unknown keys or non-numeric values raise `ValueError` (caught in `urdFusionMain` and shown as a messageBox). Values set for inapplicable joint types (e.g. effort on a fixed joint) are silently ignored. Exposes `collectJointsData(link_names, base_link)` which returns `(joints, child_visual_origins)` — `joints` is a list of `JointData` in BFS order from base_link; `child_visual_origins` is `{link_name: VisualOrigin}` giving the visual mesh origin for each non-base link in its joint frame.
...
**v1.0 complete.** Hello World scaffold → toolbar button → link selection dialog → validation → `getUniqueLinkNames` with ROS name sanitization → CSV export → STL export (meters, local frame, hidden-body warning) → URDF XML export with links and joints → "Export STLs" checkbox → material color assignment (material/rainbow/single-color modes) with URDF `<material>` elements → custom collision meshes (`urdfCollision`/`urdfSameCollision` body naming convention) → codebase refactor (shared utils constants/matToRPY, VisualOrigin dataclass, settings cache, closure-based dialog handlers, type hints throughout) → per-joint simulation parameters (damping, friction, effort, velocity) via `||key=value` joint name suffix → toolbar icon (`resources/`) and promoted button → v1.0 README with full usage documentation.

Next: see Upcoming Features in README.
...
**Per-joint simulation parameters**: users append `||key=value` pairs to the Fusion joint name (double-click to rename in the browser), e.g. `elbow_joint||d=0.1 f=0.05 e=50 v=3.14`. Keys: `d` = damping, `f` = friction, `e` = effort, `v` = velocity. Extra whitespace around `||` and between pairs is ignored. The parsed values are stored in `JointData.params` (`JointParams` dataclass). `damping`/`friction` default to `None` and are omitted from `<dynamics>` when absent; `effort`/`velocity` default to `_DEFAULT_EFFORT`/`_DEFAULT_VELOCITY` (100.0) and always appear in `<limit>` for revolute, prismatic, and continuous joints (continuous joints emit `<limit effort="..." velocity="..."/>` without lower/upper). Fixed joints emit no `<limit>`. The CSV effort/velocity columns show `-` only for fixed joints. The URDF joint name is taken from the part before `||`, sanitized normally. Parse errors (unknown key, non-numeric value) raise `ValueError` caught in `urdFusionMain`, aborting the export with a messageBox.
